...
async def anonim(event):
    ...
    async for usr in client.iter_participants(event.chat_id):
     ...
     tesdiq = await yeni_etiraf.reply("itiraf onaylansın mı ?", buttons=(
                      [
                       Button.inline("✅ Evet", data="tesdiq"
                       ),
                       Button.inline("🗑️ Sil", data="sil")
                      ]
                    ),
                    link_preview=False)
    await client.send_message(log_qrup, f"ℹ️ {gonderen} __Anonim itiraf Yazdı__")
...
async def aciq(event):
    ...
    async for usr in client.iter_participants(event.chat_id):
     ...
     tesdiq = await yeni_etiraf.reply("itiraf onaylansın mı ?", buttons=(
                    ...
                    link_preview=False)
    await client.send_message(log_qrup, f"ℹ️ {etiraf_eden} __Açık itiraf Yazdı__")
    await event.edit(f"{gonderildi}", buttons=(
                      [
                       Button.inline("💌 Yeni itiraf", data="etiraf"),
                       Button.inline("🏠 Ana Sayfa", data="start")
...
async def sil(event):
    ...
    if not tesdiq.is_reply:
      return await tesdiq.edit("itiraf silinemedi")
    if tesdiq.is_reply:
